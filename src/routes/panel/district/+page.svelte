<script>
	...
	const getDistricts = async () => {
		let getDistrictsResponse = await RestService.getDistricts(limit, skip, city, state);
		if (getDistrictsResponse['status']) {
			...
			totalDataCount = getDistrictsResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Mahalle</h3>
					</div>
				</div>
			</div>
							...
							{#if districts}
								{#each districts as district}
									<tr>
										...
										>
											{district?.city?.name?.tr ? district?.city.name.tr : '-'}
										</td>
										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{district?.state?.name?.tr ? district?.state.name.tr : '-'}
										</td>
										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{district?.order ? district?.order : '-'}
										</td>
										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{district?.name?.tr ? district?.name?.tr : '-'}
										</td>

										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{district?.name?.en ? district?.name?.en : '-'}
										</td>

										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{district?.name?.ru ? district?.name?.ru : '-'}
										</td>
										...
										>
											<button
												...
												type="button"
												on:click={goto(`/panel/district/update/${district?._id.toString()}`)}
											>
												Düzenle
											</button>
											<button
												on:click={() => deleteDistrictApprove(district?._id.toString())}
												class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
												type="button"
											>
												Sil
