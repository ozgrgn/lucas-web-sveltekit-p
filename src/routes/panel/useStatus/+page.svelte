<script>
	...
	const getUseStatuss = async () => {
		let getUseStatussResponse = await RestService.getUseStatuss(limit, skip);
		if (getUseStatussResponse['status']) {
			...
			totalDataCount = getUseStatussResponse['count'];
		} else {
			...
			<div class="block w-full overflow-x-auto">
				{#if useStatuss}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each useStatuss as useStatus}
								<tr>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{useStatus?.order ? useStatus?.order : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{useStatus?.name?.tr ? useStatus?.name?.tr : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{useStatus?.name?.en ? useStatus?.name?.en : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{useStatus?.name?.ru ? useStatus?.name?.ru : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/useStatus/update/${useStatus?._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteUseStatusApprove(useStatus?._id.toString())}
											class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
