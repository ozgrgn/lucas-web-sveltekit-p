			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Spor İmkanı</h3>
					</div>
				</div>
			</div>
			<div class="block w-full overflow-x-auto">
				{#if otoparks}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each otoparks as otopark}
								<tr>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{otopark?.order ? otopark?.order : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{otopark?.name?.tr ? otopark?.name?.tr : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{otopark?.name?.en ? otopark?.name?.en : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{otopark?.name?.ru ? otopark?.name?.ru : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/otopark/update/${otopark?._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteOtoparkApprove(otopark?._id.toString())}
											class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
