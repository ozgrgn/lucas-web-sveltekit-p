<script>
	...
	const getRentTimes = async () => {
		let getRentTimesResponse = await RestService.getRentTimes(limit, skip);
		if (getRentTimesResponse['status']) {
			...
			totalDataCount = getRentTimesResponse['count'];
		} else {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Kira Süresi</h3>
					</div>
				</div>
			</div>
			<div class="block w-full overflow-x-auto">
				{#if rentTimes}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each rentTimes as rentTime}
								<tr>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{rentTime?.order ? rentTime?.order : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{rentTime?.name?.tr ? rentTime?.name?.tr : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{rentTime?.name?.en ? rentTime?.name?.en : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{rentTime?.name?.ru ? rentTime?.name?.ru : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/rentTime/update/${rentTime?._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteRentTimeApprove(rentTime?._id.toString())}
											class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
