<script>
	...
	const getTapuTransfers = async () => {
		let getTapuTransfersResponse = await RestService.getTapuTransfers(limit, skip);
		if (getTapuTransfersResponse['status']) {
			...
			totalDataCount = getTapuTransfersResponse['count'];
		} else {
...
<div class="flex flex-wrap mt-4 h-screen relative">
	<div class="w-full mb-12 px-2 lg:px-4 ">
		<button
			...
			type="button"
			on:click={() => {
				goto('/panel/tapuTransfer/create');
			}}
		>
			<i class="fa fa-plus" />
			Ekle
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Tapu Durumu</h3>
					</div>
				</div>
			</div>
			<div class="block w-full overflow-x-auto">
				{#if tapuTransfers}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each tapuTransfers as tapuTransfer}
								<tr>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuTransfer?.order ? tapuTransfer?.order : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuTransfer?.name?.tr ? tapuTransfer?.name?.tr : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuTransfer?.name?.en ? tapuTransfer?.name?.en : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuTransfer?.name?.ru ? tapuTransfer?.name?.ru : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/tapuTransfer/update/${tapuTransfer?._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteTapuTransferApprove(tapuTransfer?._id.toString())}
											class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
