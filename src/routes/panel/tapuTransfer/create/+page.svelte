<script>
	...
	const addTapuTransfer = async () => {
		...
		let response = await RestService.addTapuTransfer(data);
		if (response['status']) {
		...
		>
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="text-center flex justify-between">
					<h3 class="font-semibold text-lg text-blueGray-700">Tapu Durumu Ekle</h3>
				</div>
			</div>
