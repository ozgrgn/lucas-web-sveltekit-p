<script>
	...
	const deleteProjectApprove = (projectId) => {
	...
	const getProjects = async () => {
		let getProjectsResponse = await RestService.getProjects(limit, skip);
		if (getProjectsResponse['status']) {
			console.log(getProjectsResponse, 'getProjectsResponse');
			projects = getProjectsResponse['projects'];
			totalDataCount = getProjectsResponse['count'];
			console.log(getProjectsResponse, 'getProjects');
		} else {
			ToastService.error($TranslateApiMessage(getProjectsResponse.message));
		}
	};
	...
	const deleteProject = async (projectId) => {
		console.log('delete');
		let response = await RestService.deleteProject(projectId);
		if (response['status']) {
			...
			<div class="rounded-t mb-0 px-4 py-3 border-0">
				<div class="flex flex-wrap items-center">
					<div class="relative w-full px-4 max-w-full flex-grow flex-1">
						<h3 class="font-semibold text-lg text-blueGray-700">Projeler</h3>
					</div>
				</div>
			</div>
								...
								{#each projects as project}
									<tr>
										...
										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{project?.name?.tr ? project?.name?.tr : '-'}
										</td>

										<td
											class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
										>
											{project?.staff?.fullName ? project?.staff?.fullName : '-'}
										</td>
										...
										>
											<button
												...
												type="button"
												on:click={goto(`/panel/project/update/${project?._id.toString()}`)}
											>
												Düzenle
											</button>
											<button
												on:click={() => deleteProjectApprove(project?._id.toString())}
												class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
												type="button"
											>
												Sil
