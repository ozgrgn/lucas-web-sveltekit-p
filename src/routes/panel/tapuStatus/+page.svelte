<script>
	...
	const getTapuStatuss = async () => {
		let getTapuStatussResponse = await RestService.getTapuStatuss(limit, skip);
		if (getTapuStatussResponse['status']) {
			...
			totalDataCount = getTapuStatussResponse['count'];
		} else {
			...
			<div class="block w-full overflow-x-auto">
				{#if tapuStatuss}
					<table class="items-center w-full bg-transparent border-collapse">
						...
						<tbody>
							{#each tapuStatuss as tapuStatus}
								<tr>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuStatus?.order ? tapuStatus?.order : '-'}
									</td>
									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuStatus?.name?.tr ? tapuStatus?.name?.tr : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuStatus?.name?.en ? tapuStatus?.name?.en : '-'}
									</td>

									<td
										class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 text-center"
									>
										{tapuStatus?.name?.ru ? tapuStatus?.name?.ru : '-'}
									</td>
									...
									>
										<button
											...
											type="button"
											on:click={goto(`/panel/tapuStatus/update/${tapuStatus?._id.toString()}`)}
										>
											Düzenle
										</button>
										<button
											on:click={() => deleteTapuStatusApprove(tapuStatus?._id.toString())}
											class="bg-white text-primary hover:bg-[#6e6e85] hover:text-white border border-primary rounded font-bold  text-sm px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
											type="button"
										>
											Sil
