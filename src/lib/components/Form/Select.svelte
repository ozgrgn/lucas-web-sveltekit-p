...
<div class="relative group">
	<select
		class=" px-2 placeholder-black/50 disabled:bg-white disabled: placeholder-black/20 text-black {value !=
		undefined
			? 'font-normal'
			: 'font-normal text-black/40'} customPanel  rounded  shadow group-hover:border-secondary  focus:outline-0  {customClass} {customClass
			? ''
			: 'h-9'} disabled:bg-slate-100"
	...
	{#if label}
		<div
			class="absolute bottom-10 md:bottom-10 z-30 ml-2 px-1 bg-white text-xs text-black/50 group-hover:text-secondary {value?"text-secondary":""}"
		>
			{label}
		</div>
	{/if}
